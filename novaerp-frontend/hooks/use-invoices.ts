...
export function useCustomerInvoices(page = 0, size = 10, status?: CustomerInvoiceStatus, clientId?: number, saleOrderId?: number, options?: { enabled?: boolean }) { return useQuery({ queryKey: ["customer-invoices", page, size, status, clientId, saleOrderId], queryFn: () => getCustomerInvoices(page, size, status, clientId, saleOrderId), enabled: options?.enabled ?? true }); }
export function useSupplierInvoices(page = 0, size = 10, status?: SupplierInvoiceStatus, supplierId?: number, purchaseOrderId?: number, options?: { enabled?: boolean }) { return useQuery({ queryKey: ["supplier-invoices", page, size, status, supplierId, purchaseOrderId], queryFn: () => getSupplierInvoices(page, size, status, supplierId, purchaseOrderId), enabled: options?.enabled ?? true }); }
